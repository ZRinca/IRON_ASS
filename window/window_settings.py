            ...
            for event in pygame.event.get():
                if event.type == pygame.MOUSEBUTTONDOWN:
                    if button_menu_left.collidepoint(event.pos):
                        if entered_language < len(self.language) - 1: entered_language += 1
                    if button_menu_right.collidepoint(event.pos):
                        if entered_language > 0: entered_language -= 1
                    ...
                    if button_menu.collidepoint(event.pos):
                        return "menu", entered_language

            # off/on music
            self.screen.blit(button_true if self.music_off_on else button_false, button_music_on_off)
            self.screen.blit(self.font.render(f"{self.music_text_language[entered_language]}", True, (255, 255, 255)), (625, 220))

            # settings music
            self.screen.blit(button_left, button_menu_left)
            self.screen.blit(self.font.render(f"{self.language[entered_language]}", True, (255, 255, 255)), (640, 70))
            self.screen.blit(button_right, button_menu_right)

            # button menu
            self.screen.blit(button, button_menu)
            self.screen.blit(self.font.render(f"{self.menu_button_language[entered_language]}", True, (255, 255, 255)), (150, 250))

            # button accept
            self.screen.blit(button, button_accept)
            self.screen.blit(self.font.render(f"{self.accept_button_language[entered_language]}", True, (255, 255, 255)), (150, 350))
